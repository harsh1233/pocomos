...
use App\Mail\CustomerSendExportEmail;
...
class SendEmailCustomerExport implements ShouldQueue
{
    use Dispatchable;
    ...
    {
        Log::info("SendEmailCustomerExport Job Started For Export");
        ...
        if (count($this->customerIds)) {
            $this->customerIds = $this->convertArrayInStrings($this->customerIds);
            $whereSql .= " AND pc.id IN($this->customerIds) ";
        }
        ...
        $data_new = $this->convert_csv_formate_customer_data($heading, $data, $this->columns);

        $filename = storage_path('app/public/pdf/Customers.csv');
        ...
        $alert_details['description'] = 'The Customer Export has completed successfully <br><br><p>The export has been mailed to '.$this->request_data['recipient'].'</p>';
        ...
        $email->send(new CustomerSendExportEmail($this->request_data));

        /**Unlink csv file */
        unlink($filename);
        Log::info("SendEmailCustomerExport Job End For Export");
    }
}
