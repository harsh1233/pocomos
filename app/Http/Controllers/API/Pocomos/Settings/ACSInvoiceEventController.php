...
class ACSInvoiceEventController extends Controller
{
    use Functions;
    ...
    public function list(Request $request)
    {
        ...
        $invoiceDueEvents = PocomosAcsEvent::with('exception_tags.exception_tag_detail', 'acs_agreements.agreement_detail', 'acs_serice_types.service_type_detail', 'acs_tags.tag_detail')
            ...
            ->where('pocomos_acs_events.event_type', 'Invoice Due')
            ->where('pocomos_acs_events.office_id', $request->office_id)->where('pocomos_acs_events.active', 1)
            ->select('pocomos_acs_events.*', 'ca.title as EmailLetter', 'sms.title as SMSLetter')
            ->get()->toArray();

        usort($invoiceDueEvents, function ($a, $b) {
        ...
        $jobEvents_test = array_slice($invoiceDueEvents, $perPage * ($page - 1), $perPage);
        ...
        return $this->sendResponse(true, 'invoice alert List', [
            'invoice_due_events' => $invoiceDueEvents,
            'count' => $count,
        ]);
    }
    ...
    public function create(Request $request)
    {
        $v = validator($request->all(), [
            ...
            'event_type' => 'required|in:Invoice Due',
            'autopay' => 'nullable',
            'customer_autopay' => 'nullable',
